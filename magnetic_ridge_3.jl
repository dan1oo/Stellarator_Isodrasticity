...
const FOOTPRINT_OUTER_PAD = parse(Float64, get(ENV, "MAGRID3_FOOTPRINT_OUTER_PAD", "0.07"))
const FOOTPRINT_INNER_PAD = parse(Float64, get(ENV, "MAGRID3_FOOTPRINT_INNER_PAD", "0.04"))
...
const XY_RADIUS_MAX = parse(Float64, get(ENV, "MAGRID3_XY_RADIUS_MAX", "0.25"))
const MIN_NEIGHBORS = parse(Int, get(ENV, "MAGRID3_MIN_NEIGHBORS", "18"))
const Z_PAD = parse(Float64, get(ENV, "MAGRID3_Z_PAD", "0.01"))

# Field validity and ridge check
...
@inline function theta_bin(theta::Float64, ntheta::Int)
    # map theta in (-pi, pi] to [1, ntheta]
    t = (theta + π) / (2π)
...
function _fill_circular_nearest!(v::Vector{Float64}; is_max::Bool = false)
    ...
    for i in eachindex(v)
        ...
        # nearest finite bin on circular index
        best_j = finite_idx[1]
...
function build_xy_footprint(cx::Vector{Float64}, cy::Vector{Float64}; ntheta::Int = FOOTPRINT_NTHETA)
    ...
    # light circular smoothing to prevent jagged angular aliasing
    rmin_s = similar(rmin)
    rmax_s = similar(rmax)
    for i in 1:ntheta
        i0 = mod1(i - 1, ntheta)
...
function save_xy_mask_debug_figure(
    ...
    ax = Axis(
        fig[1, 1];
        title = "2D XY footprint mask used by magnetic_ridge_3",
        xlabel = "X (m)",
        ylabel = "Y (m)",
        aspect = DataAspect(),
    )
    ...
    out = joinpath(@__DIR__, "magnetic_ridge_3_mask2d.png")
    save(out, fig)
...
function map_ridge_surface(
    ...
    # Columns where >1 valid ridge root exists: store (x,y), all valid z, and chosen z (same as grid)
    multi_ridge_xy_x = Float64[]
    multi_ridge_xy_y = Float64[]
    multi_ridge_valid_zs = Vector{Vector{Float64}}()
    multi_ridge_chosen_z = Float64[]

    grid_x = Float64[]
    grid_y = Float64[]
    grid_z = Float64[] # NaN where no valid point
    ...
    for x in xs
        for y in ys
            ...
            z_pick = valid_z[k]
            if length(valid_z) > 1
                push!(multi_ridge_chosen_z, z_pick)
            end
    ...
    println("No valid ridge root in local confinement interval: $no_root")
    println("XY columns with multiple valid roots in confinement interval: $multi_root_xy")

    return ridge_x, ridge_y, ridge_z, grid_x, grid_y, grid_z,
        multi_ridge_xy_x, multi_ridge_xy_y, multi_ridge_valid_zs, multi_ridge_chosen_z
end
...
function main()
    ...
    if MASK_ONLY
        println("MASK_ONLY=true: skipped ridge root search.")
        return
    end

    rx, ry, rz, gx, gy, gz, mux, muy, muzs, muchosen = map_ridge_surface(
        coils,
        idx,
        fp,
        x_min,
    ...
    ax = Axis3(
        fig[1, 1];
        title = "Magnetic ridge in confinement zone (NaN-masked XY grid)",
        xlabel = "X (m)",
        ylabel = "Y (m)",
        zlabel = "Z (m)",
        aspect = :data,
    ...
    # Ridge points
    Rcol = sqrt.(rx .^ 2 .+ ry .^ 2)
    scatter!(ax, rx, ry, rz; markersize = 7, color = Rcol, colormap = :plasma, alpha = 0.9)

    # Optional: faint confinement cloud for context (downsampled for speed)
    ...
    # Multi-root columns: every valid z (gold) — primary ridge uses argmax |B| per column
    if !isempty(muzs)
        mx_all = Float64[]
        my_all = Float64[]
        mz_all = Float64[]
    ...
    out_png = joinpath(@__DIR__, "magnetic_ridge_3.png")
    save(out_png, fig)
    println("Saved figure to ", out_png)

    # 2D: XY locations where multiple valid ridge roots occurred
    if !isempty(mux)
        fig2 = Figure(size = (900, 800))
        ax2 = Axis(
            fig2[1, 1];
            title = "XY grid cells with multiple valid ridge roots (within confinement z-interval)",
            xlabel = "X (m)",
            ylabel = "Y (m)",
            aspect = DataAspect(),
        )
        scatter!(ax2, mux, muy; markersize = 14, color = (:darkorange, 0.95), strokewidth = 1.5, strokecolor = :black)
        out_multi_xy = joinpath(@__DIR__, "magnetic_ridge_3_multi_xy.png")
        save(out_multi_xy, fig2)
        println("Saved multi-root XY map to ", out_multi_xy)
    end

    # Save grid products including NaN map (never overwrite: magnetic_ridge_3.jld2 → magnetic_ridge_3_1.jld2, …)
    out_jld2_base = joinpath(@__DIR__, "data", "magnetic_ridge3", "magnetic_ridge_3.jld2")
    ...
    JLD2.jldopen(out_jld2, "w") do f
        ...
        f["grid_y"] = gy
        f["grid_z"] = gz
        f["multi_ridge_xy_x"] = mux
        f["multi_ridge_xy_y"] = muy
        f["multi_ridge_valid_zs"] = muzs
        f["multi_ridge_chosen_z"] = muchosen
        f["meta"] = Dict(
            ...
            "z_pad" => Z_PAD,
            "multi_ridge_note" =>
                "multi_ridge_*: only rows where >1 valid ridge root; valid_zs is ragged Vector{Vector}; chosen_z matches primary ridge_z/ridge_*",
        )
    end
